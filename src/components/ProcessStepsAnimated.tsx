        ...
        {steps.map((step, index) => (
          ...
          >
            <div className="flex justify-center mb-4 group/step">
              <div
                className="w-9 h-9 rounded-full border border-gray-300 text-gray-400 bg-white flex items-center justify-center text-sm font-semibold shrink-0 transition-all duration-1000 ease-out group-[.in-view]:border-blue-600 group-[.in-view]:text-blue-600 group-[.in-view]:scale-105 group-hover/step:border-blue-500 group-hover/step:-translate-y-0.5"
                style={{ transitionDelay: `${DELAYS[index]}ms` }}
              >
                {index + 1}
              </div>
            </div>
            <p className="text-[15px] sm:text-base font-medium text-gray-900 mb-1.5 transition-colors group-hover:text-gray-800">
              {step.title}
            </p>
            <p className="text-[13.5px] sm:text-sm text-gray-600 max-w-[240px]">
              {step.detail}
            </p>
          </div>
        ))}
